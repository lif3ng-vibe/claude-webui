...
- **右键选 provider 启动**（一次性绑定，不持久化）：`providerEnv(id)` 把 provider 解析为 env（`ANTHROPIC_BASE_URL`/`AUTH_TOKEN`/`API_KEY`/`MODEL`），**经 `claude --settings` 注入**（机制见下）。四个按钮支持右键 → `useProviderMenu`/`ProviderMenu` 选 provider：
  - `+ 新会话`（右键预置 provider 进对话框）、续接 `发送`（`/run` body 带 providerId）、🖥 终端（URL `?provider=`）、📋 复制命令（`GET /api/projects/:dir/sessions/:sid/copy-command?provider=`，后端 `buildResumeCommand` 生成 `claude --settings '<json>'` 命令，前端无密钥；弹确认含 token）。
