...
- **页面快捷键**（`web/src/App.vue` 全局 keydown，三端通用）：① 缩放 `Ctrl/Cmd + +/-/0`——`display` store 的 `zoom`（`useStorage` 持久化，0.5–2.0），落到 `:root` 的 `--zoom`/`--zoom-inv`；`styles.css` 里 `#app { zoom: var(--zoom); height: calc(100vh / var(--zoom)); }`（calc 抵消全屏布局的缩放溢出），`.titlebar`/`.topnav` 用 `--zoom-inv` 反向缩放保持原尺寸（即"标题栏+主导航固定，其余缩放"）。② DevTools `F12`/`Ctrl(Cmd)+Shift+I`——桌面壳走 bridge `openDevTools`（Electron `webContents.openDevTools()` 停靠、Tauri `WebviewWindow::open_devtools()`），web 不拦截走浏览器原生；Tauri 侧 `desktop_open_devtools` 同样需在 `permissions/commands.toml` + capability 授权（remote origin ACL），并依赖 Cargo.toml 的 `devtools` feature。
