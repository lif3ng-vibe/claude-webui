...
- **快捷应答浮层**（`display.quickReply` 开关，默认关；`useStorage` → localStorage 同源多窗口经 `storage` 事件同步）：开启后，run 结束对最近一次 assistant 文本做启发式抽取（`composables/useInteractionPicker`：选项列表 / 是·否 / 继续，按优先级取一种），以可拖动浮层 `InteractionPicker.vue` 给出大按钮，点选 → 直发下一条 prompt（跳过确认）。主 SessionsView 与独立 SessionPage 两个会话窗口共用同一 composable，故开关对全部会话窗口生效。注：headless（`-p`）模式无结构化多选事件，只能对文本近似抽取；真正的"上下选+回车"TUI 菜单只在交互式终端（§11）里、需解析 ANSI，未做。
...
- **目录内新建会话**（不是 resume）：`+ 新会话` 按钮（左侧栏 header + DirPage header）弹 `NewSessionDialog`——选工作目录（桌面端 `pickDirectory` 原生文件夹框，web 端从 `/api/projects` 已用目录列表点选或手输）+ 模式（单发首条指令 / 交互式终端）+ **provider 下拉**（默认=不注入走 claude settings.json，或任一 provider 经 `--settings` 注入）。`+ 新会话` 按钮除左侧栏 header + DirPage header 外，主页每个目录行右侧也各有一个（点击预填该目录 cwd 开建）。
...
- **右键选 provider 启动**（一次性绑定，不持久化）：`providerEnv(id)` 把 provider 解析为 env（`ANTHROPIC_BASE_URL`/`AUTH_TOKEN`/`API_KEY`/`MODEL`），**经 `claude --settings` 注入**（机制见下）。新建会话走对话框内 provider 下拉；续接 `发送`/🖥 终端/📋 复制命令三按钮支持右键 → `useProviderMenu`/`ProviderMenu` 选 provider：
  - `+ 新会话`：对话框内 provider 下拉选；外层「+」与目录行「+」右键 = 预选 provider 进对话框（`NewSessionDialog` 加 `defaultDir`/`defaultProviderId` prop，`watch(show)` 打开时重置）。续接 `发送`（`/run` body 带 providerId）、🖥 终端（URL `?provider=`）、📋 复制命令（`GET /api/projects/:dir/sessions/:sid/copy-command?provider=`，后端 `buildResumeCommand` 生成 `claude --settings '<json>'` 命令，前端无密钥；弹确认含 token）这三个按钮仍右键选 provider。
  - 左键 = 默认：不传 `--settings`，claude 按它自身配置加载（即 `~/.claude/settings.json` 的 env 块，cc-switch 当前选中），**不是** claude-webui 的"活动 provider"。
- **provider 注入机制（为何用 `--settings` 而非 spawn env）**：Claude Code 启动时用 `~/.claude/settings.json` 的 `env` 块**覆盖**进程环境变量（cc-switch 等工具靠此切换 provider），故把 provider env merge 进 spawn env 会被覆盖、不生效（实测 `/status` 仍显示 cc-switch 的 provider）。而 `claude --settings <file-or-json>` CLI 参数的 env 优先级**高于** `~/.claude/settings.json`（实测 model 标记确认），故经此注入才能盖过 cc-switch：
  - 右键启动（`ClaudeRunner.run`/`runNew`、`TerminalManager`）：`writeProviderSettings` 把 `{"env":{...}}` 写临时文件，spawn 加 `--settings <临时文件>`，run 结束 `delProviderSettings` 删除。文件形式避开 Windows cmd.exe 对内联 JSON 引号的解析。
  - 📋 复制命令（`buildResumeCommand`）：内联 JSON `claude --settings '{"env":{...}}' --resume <sid>`（bash 单引号包裹；用户粘到自己 shell 跑，临时文件不在其路径）。
  - 已知边界：临时文件路径若含空格（如用户名带空格），`shell:true` 下裸路径会被 cmd.exe 拆分——当前用户路径无空格，未处理该边界。
- **飞书 provider**（应用级持久化，是该一次性规则的例外）：`FeishuApp.providerId` + `/provider [名称|id|off]` 命令；`/new` 与续接都经 `--settings` 注入 `providerEnv(app.providerId)`。`matchProvider` 按名称/id 匹配。
...
```
config.ts              # 配置：providers 数组 + active + env 兜底；resolveProvider(id)；providerEnv(id)/matchProvider
...
claude/
  ...
  Runner.ts             # ClaudeRunner：包 claude --resume/新建 一次性子进程；provider env 经 --settings 注入；streamChildEvents/extractSessionId
  resumeCommand.ts      # buildResumeCommand：拼含 provider 的复制命令（--settings '<json>'）
  cwdGuard.ts           # assertSafeCwd：校验新建会话工作目录
  providerSettings.ts   # writeProviderSettings/delProviderSettings：provider env 写/删临时 settings 文件（供 --settings）
terminal/
  TerminalManager.ts    # 网页交互终端：node-pty 跑 claude（resume/new）+ WebSocket 收发/resize/锁（§11）；provider env 经 --settings 注入
...
- **兼容性接口**：shell 在页面加载前注入 `window.__claudeWebuiDesktop = { kind, openWindow, minimize, toggleMaximize, close, setAlwaysOnTop, isAlwaysOnTop, pickDirectory, service: { status, start, stop, restart, getLogs, onLog } }`。Electron 用 preload+`contextBridge`；Tauri 用 `initialization_script` 包 `__TAURI__`（`withGlobalTauri:true`）。web 下不存在 → `openWindow` 回退 `window.open`、`pickDirectory` 回退 `null`（调用方转输入框），标题栏/服务页隐藏。前端 `web/src/lib/desktop.ts` 读该对象暴露 `isDesktop`/`openWindow`/窗口控制/`alwaysOnTop`/`pickDirectory`；`openWindow.ts` 转发到它，6 处调用点签名不变。`pickDirectory`：Electron `dialog.showOpenDialog(openDirectory)`；Tauri `desktop_pick_directory` 命令用 `rfd` crate（原生文件夹框，纯 crate 与现有自定义命令模式一致）。
- **Tauri IPC 授权（capabilities）**：本应用前端由本地 sidecar 以 `http://localhost:<随机端口>` 提供（`WebviewUrl::External`，非 `frontendDist`），在 Tauri 2 下被判为 **remote origin**。Tauri 2 对 remote origin 的**自定义命令强制 ACL**（`webview/mod.rs`：`!is_local` → "remote content can never reach custom commands unless an explicit `remote` capability has been configured for them"）；dev 因 `debug_assertions` 下 ACL 宽松、且 devUrl 被判 local 而不暴露。故生产环境必须三件齐备（`src-tauri/capabilities/default.json`）：① `windows:["*"]` 覆盖运行时动态创建的 terminal/projects/conversations 等窗口 label；② `remote.urls:["http://localhost:*/*","http://127.0.0.1:*/*"]` 授权 sidecar 远端 origin；③ **为每个自定义命令定义权限并在 capability 引用**——`src-tauri/permissions/commands.toml` 用 `[[permission]] identifier="allow-<kebab>" commands.allow=["<snake>"]` 定义 `desktop_open_window`/`desktop_pick_directory`/`service_*` 等 12 个命令，capability `permissions` 列出对应 `allow-*`。缺③则 bridge 调用被拒 "Command … not allowed by ACL" → `isDesktop=false`、`openWindow` 回退 `window.open`、新窗口/终端打不开（v0.0.10 的根因）。
...
- **页面快捷键**（`web/src/App.vue` 全局 keydown，三端通用）：① 缩放 `Ctrl/Cmd + +/-/0`——`display` store 的 `zoom`（`useStorage` 持久化，0.5–2.0），落到 `:root` 的 `--zoom`/`--zoom-inv`；`styles.css` 里 `#app { zoom: var(--zoom); height: calc(100vh / var(--zoom)); }`（calc 抵消全屏布局的缩放溢出），`.titlebar`/`.topnav` 用 `--zoom-inv` 反向缩放保持原尺寸（即"标题栏+主导航固定，其余缩放"）。**全屏滚动条修复**：`html/body/#app` 加 `overflow:hidden`（整屏不滚动，各页面内部区域自滚）；`App.vue` 根改用显式 `height: calc(100vh / var(--zoom))` 而非 `h-full`——`NConfigProvider`/`NMessageProvider` 包裹层的 `height:auto` 会断开 `h-full` 百分比链（子级 `h-full=100%` 解析成 auto → 内容溢出 body），显式视口高度绕开它，下游 `flex-1 min-h-0` + 各页 `overflow-auto` 才能正确撑满与内部滚动。② DevTools `F12`/`Ctrl(Cmd)+Shift+I`——桌面壳走 bridge `openDevTools`（Electron `webContents.openDevTools()` 停靠、Tauri `WebviewWindow::open_devtools()`），web 不拦截走浏览器原生；Tauri 侧 `desktop_open_devtools` 同样需在 `permissions/commands.toml` + capability 授权（remote origin ACL），并依赖 Cargo.toml 的 `devtools` feature。
...
- **macOS 图形应用 PATH 极简（坑，曾导致终端 `posix_spawnp failed`）**：打包后的桌面 app 从 Dock/Finder/launchd 启动，继承的 PATH 极简（仅 `/usr/bin:/bin:/usr/sbin:/sbin`），找不到用户经 homebrew/npm 等安装的 `claude`/`git`/`node`。`src/server/shellEnv.ts` 在 server 进程启动最早期（`index.ts` 首个 import）用 `$SHELL -l -c 'echo $PATH'` 同步拉取登录 shell 的完整 PATH 注入 `process.env`，登录 shell 失败时回退常见路径列表（homebrew/npm/cargo/bun/deno 等）；后续 `node-pty.spawn` 继承修正后的 PATH。仅 macOS/Linux 需要，Windows 图形应用继承系统全局 PATH（v0.0.17）。
- **node-pty spawn-helper 无执行权限（坑，曾导致终端 `posix_spawnp failed`，v0.0.18 真正根因）**：node-pty 在 macOS 上**不直接 fork+exec**，而是用 `posix_spawn` 启动一个 `spawn-helper` 二进制（`pty.cc`：`argv[0]=helperPath`），helper 内部再 `execvp` 跑 `claude`。但 npm 包里 `prebuilds/<plat-arch>/spawn-helper` 默认 `-rw-r--r--`（无执行位），打包后也未恢复 → `posix_spawn` 报 `EACCES` → `PtyFork` 抛 `"posix_spawnp failed."`（注意报错名带 `p` 但实际用的是无 `p` 的 `posix_spawn`，**不搜索 PATH**，故上一条 PATH 修复不充分）。`src/server/fixPtyExec.ts` 在 server 启动最早期（`index.ts` 次优先 import，先于 node-pty 加载）用 `require.resolve('node-pty/package.json')` 定位包目录，对其下 `build/Release` 与 `prebuilds/<plat-arch>` 的 `spawn-helper` `chmod 0o755`。
- **Electron app.asar.unpacked 路径下 posix_spawn 被拒（坑，Electron 版 `posix_spawnp failed` 真正根因，Tauri 无此问题）**：即便 spawn-helper 已 `+x`+签名，只要它在 `app.asar.unpacked/` 路径下，macOS 的 `posix_spawn` 仍拒绝执行（复制到 `/tmp`、`~/.claude-webui/cache/` 等普通目录即恢复）。Tauri 的 spawn-helper 在 `Contents/Resources/` 下不受影响。解法：`src/server/fixPtyAsar.ts` 在 server 启动最早期把 node-pty 整包 `cp -R` 到 `~/.claude-webui/cache/node_modules/node-pty`（普通目录），并 patch `Module._resolveFilename` 把 `require('node-pty')` 重定向到缓存副本。**关键**：`src/terminal/TerminalManager.ts` 必须用 `createRequire(import.meta.url)('node-pty')`（CJS）而非 `import * as nodePty from 'node-pty'`——esbuild ESM bundle 会把后者保留为 ESM `import`，绕过 CJS `_resolveFilename` patch，使重定向失效。
...
- **命令**：`/sessions` `/use` `/info` `/new <目录> <指令>` `/provider [名称|id|off]` `/stop` `/help`；纯文本续接当前 session。`/new` 在指定 cwd 创建新 session（`claude -p`，不带 `--resume`），从 stream-json 提取新 sessionId 设为当前。`/provider` 设/清应用级 provider（`FeishuApp.providerId`，`matchProvider` 按名称/id 匹配），`/new` 与续接都经 `claude --settings` 注入 `providerEnv(app.providerId)`（见 §4.11）。
