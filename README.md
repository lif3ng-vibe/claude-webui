...
6. **飞书机器人（远程续接 + 通知）** — 支持多个飞书应用，每个绑定一个 Claude session；在飞书发消息即续接该 session，结果以交互卡片增量流式回传；本地任务完成也推飞书。命令 `/sessions` `/use` `/info` `/new <目录>` `/provider [名称|id|off]` `/stop`；白名单 + 首个发消息者自动认主 + 上线主动私聊。
